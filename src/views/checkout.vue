                    ...
                    <div class="box-inner js-checkout-address-panel ">
                        <div class="address-common-table js-multiple-address-panel">
                            <ul class="address-item-list clear js-address-item-list">
                                <li v-for="option,num in $store.state.receiveInfo"
                                    :class="{'selected-address-item':num===addIndex}"
                                    class="js-choose-address"
                                    @click="tabAddIndex(num)">
                                    <div class="address-item">
                                        <div class="name-section">  {{option.name}}  </div>
                                        <div class="mobile-section">{{option.phone}}</div>
                                        <div class="detail-section"> {{option.province}} {{option.city}} {{option.county}}<br> {{option.add}} </div>
                                    </div>
                        ...
                        <div class="radio-box">
                            <label>
                                ...
                                <span
                                    ...
                                    @click="chosenFn(true)"><a></a></span>  个人
                            </label>
                            <label>
                                <input type="radio" class="hide">
                                <span
                                    class="blue-radio"
                                    :class="{'blue-radio-on':!invoice.personal}"
                                    @click="chosenFn(false)"><a></a></span>  单位
                            </label>
                        </div>
                        <div
                            ...
                            <div class="module-form-item-wrapper no-icon small-item">
                                ...
                                <input type="text" class="js-verify" v-model="invoice.name" v-focus>
                            </div>
                        </div>
                    </div>
            ...
            <div class="gray-box">
                ...
                <div class="box-inner">
                    <div class="order-discount-line">
                        ...
                        <p> 运费： <span>+ ¥ {{freeFreight}}.00</span> </p>
                        <p class="discount-line js-discount-cash"> <em>现金券</em>： <span> - {{scale}} </span> </p>
                    </div>
                </div>
                <div class="box-inner">
                    <div class="last-payment clear">
                        <span class="jianguo-blue-main-btn big-main-btn payment-blue-bt fn-right js-checkout" @click="submitOrderFn"> <a>提交订单</a> </span> <span class="prices fn-right">应付金额： <em>¥ {{lastTotalPric}}.00</em></span>
                    </div>
                </div>
            </div>
        </div>
        <add-pop v-show="popShow" @closefn="closePop"></add-pop>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        computed:{
            ...
            cartData(){
                return this.$store.getters.checkedData;
            },
            //计算总价
            totalPrice(){
                return this.$store.getters.totalCheckedPri;
            },
            lastTotalPric(){
                return this.totalPrice+this.freeFreight-this.scale;
            },
            //计算是否包邮
            freeFreight(){
        ...
        methods:{
            ...
            submitOrderFn(){
                ...
                let addressChecked=this.addressInfo[this.addIndex];
                ...
                let orderData={
                    ...
                    goodsData:this.cartData,
                    price:this.totalPrice,
                    totalPric:this.lastTotalPric,
                    freight:this.freeFreight,
                    invoiceName:this.invoice.name,
                    date:iDate.getFullYear()+'-'+Month+'-'+day,
                    address:addressChecked,
                    isPay:false
                };
                this.$store.commit('submitOrder',orderData);
                this.$router.push({name:'payment',query:{'orderId':orderData.orderId}})
            }
        },
        components:{
            addPop
