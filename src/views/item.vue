                    ...
                    <div class="thumbnail">
                        <ul>
                            <li
                                ...
                                v-for="item,index in filterData.ali_images"
                                @click="tabIndex(index)"><img :src="`${item}?x-oss-process=image/resize,w_54/quality,Q_90/format,webp`">
                            </li>
                        </ul>
                    </div>
                    <div class="thumb">
                        <ul>
                            <li
                                :class="{on:index===onNum}"
                                v-for="item,index in filterData.ali_images"><img :src="`${item}?x-oss-process=image/resize,w_440/quality,Q_90/format,webp`">
                            </li>
                        </ul>
                    </div>
                </div>
            ...
            <div class="banner">
                <div class="sku-custom-title">
                    <div class="params-price">
                        <span><em>¥</em><i>{{filterData.price}}</i></span>
                    </div>
                    <div class="params-info">
                        <h4>{{filterData.title}}</h4>
                        <h6>{{filterData.sub_title}}</h6>
                    </div>
                </div>
                <div class="sku-dynamic-params-panel">
                    <div class="sku-dynamic-params clear">
                        ...
                        <ul class="params-colors">
                            <li :class="{'cur':Number(cols.id)===Number($route.query.itemId)}"
                                v-for="cols,colIndex in filterData.sku_list">
                                <router-link
                                    :to="{name:'item',query:{itemId:cols.id}}"
                                    :title="cols.color"
                                    ><img :src="`http://img01.smartisanos.cn/${cols.image}20X20.jpg`"></router-link>
                            </li>
                        </ul>
                    </div>
                    <div class="sku-dynamic-params clear">
                        ...
                        <div class="params-detail clear">
                            <div class="item-num js-select-quantity">
                                <span
                                    class="down"
                                    :class="{'down-disabled':countBer<=1}"
                                    @click="plusFn">-</span>
                                <span class="num">{{countBer}}</span>
                                <span
                                    class="up"
                                    :class="{'up-disabled':countBer>=filterData.limit_num}"
                                    @click="addFn">+</span>
                            </div>
                        </div>
                    </div>
                ...
                <div class="sku-status">
                    <div class="cart-operation-wrapper clearfix">
                        <span
                            ...
                            @click="addCart(filterData)"><a>加入购物车</a></span>
...
<script>
    ...
    export default {
        ...
        computed:{
            filterData(){
                return this.itemData.find(item=>{
        ...
        methods:{
            ...
            addCart(opt){
                console.log(opt);
                let optData={info:opt,count:this.countBer};
                this.$store.commit('addItems',optData)
            },
            //点击++
            addFn(){
                this.countBer++;
                if(this.countBer>=this.filterData.limit_num){
                    this.$store.commit('showPrompt');
                    this.countBer=this.filterData.limit_num
                }
...
<style scoped>
    ...
    .item-box .blue-title-btn{
        ...
        background: #5c85e5 linear-gradient(#779ae9,#5078df);
        color: #fff;
        cursor: pointer;
    }
