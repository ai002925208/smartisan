        ...
        <div class="content page-order-payment">
            <div class="gray-box clear">
                ...
                <div class="box-inner payment-checkout-panel clear">
                    ...
                    <span class="prices"> 应付金额：   <em><span>¥ </span>{{orderData.totalPric}}.00</em>   </span>
                </div>
            </div>
            <div class="confirm-detail">
                <div class="info-title">订单编号</div>
                <p class="info-detail">{{orderData.orderId}}</p>
            </div>
            <div class="confirm-detail">
                <div class="info-title">收货信息</div>
                <p class="info-detail">姓名：{{orderData.address.name}}</p>
                <p class="info-detail">联系电话：{{orderData.address.phone}}</p>  <p class="info-detail">详细地址：北京市市辖区海淀区上地十街辉煌国际西6号楼319室</p>
            </div>
            <div class="confirm-detail">
                <div class="info-title">发票信息</div>
                <p class="info-detail">发票类型：电子发票</p>
                <p class="info-detail">发票抬头：{{orderData.invoiceName}}</p>
            ...
            <div class="confirm-goods-table">
                <div class="cart-items clear" v-for="goods in orderData.goodsData">
                    <div class="name fn-left hide-row">
                        <div class="name-cell">
                            <a href="http://www.smartisan.com/shop/#/item/100029901" :title="`${goods.title}(${goods.spec_json.show_name})`" target="_blank">{{goods.title}}({{goods.spec_json.show_name}})</a><br>
                        </div>
                    </div>
                    <div class="subtotal fn-right">
                        <div class="subtotal-cell">¥ {{goods.price*goods.count}}.00</div>
                    </div>
                    <div class="goods-num fn-right">{{goods.count}}</div>
                    <div class="price fn-right">¥ {{goods.price}}.00</div>
                </div>
            </div>
...
<script>
    export default {
        computed:{
            orderData(){
               return this.$store.state.orderData[0];
            }
        }
    }
</script>
