                                ...
                                <div class="address-form">
                                    <div class="module-form-row">
                                        <div class="form-item-v3">
                                            <input type="text" class="js-verify" v-model="receiveData.name" placeholder="收货人姓名">
                                    ...
                                    <div class="module-form-row clear">
                                        <div class="form-item-v3 area-code-w fn-left form-valid-item">
                                            <input type="text" class="js-verify js-address-area-code" v-model="receiveData.areaCode" placeholder="区号（可选）">
                                            <div class="verify-error"></div>
                                        </div>
                                        <div class="form-item-v3 telephone-w fn-right form-valid-item">
                                            <input type="text" class="js-verify js-address-telephone" v-model="receiveData.landLine" placeholder="固定电话（可选）">
                                            <div class="verify-error"></div>
                                        </div>
                                    </div>
                                    <div class="module-form-row clear">
                                        <div class="form-item-v3 select-item province-wrapper">
                                            <select name="province_code" class="province select-province js-form-province js-verify" v-model="receiveData.provinceId">
                                                <option value="0">请选择省份</option>
                                                <option :value="province.area_id" v-for="province in addList">{{province.area_name}}</option>
                                            </select>
                                        </div>
                                    </div>
                                    <div class="module-form-row clear">
                                        <div class="form-item-v3 select-item city-wrapper fn-left form-focus-item">
                                            <select class="city select-city js-form-city js-verify" v-model="receiveData.cityId">
                                                <option value="0">请选择城市</option>
                                                <option :value="city.area_id" v-for="city in cityList">{{city.area_name}}</option>
                                            </select>
                                        </div>
                                        <div class="form-item-v3 select-item district-wrapper fn-right form-focus-item">
                                            <select class="city select-city js-form-city js-verify" v-model="receiveData.countyId">
                                                <option value="0">请选择区县</option>
                                                <option :value="county.area_id" v-for="county in countyList">{{county.area_name}}</option>
                                            </select>
                                        </div>
                                    </div>
                                    <div class="module-form-row">
                                        <div class="form-item-v3">
                                            <input type="text" class="js-verify" v-model="receiveData.add" placeholder="详细地址，如街道名称，楼层，门牌号码等">
                                            <div class="verify-error"></div>
                                        </div>
                                    </div>
                                    <div class="module-form-row fn-clear">
                                        <input type="checkbox" class="hide" v-model="receiveData.default">
                                        <span
                                            class="blue-checkbox"
                                            :class="{'blue-checkbox-on':receiveData.default}"
                                            @click="tabDefault"></span>设为默认
                                    </div>
                                    <div
                                        class="dialog-blue-btn big-main-btn js-verify-address"
                                        :class="{'disabled-btn':!judgeAll}"
                                        @click="saveAddress">
                                        <a>保存</a>
                                    </div>
                                </div>
...
<script>
    ...
    export default {
        ...
        created(){
            if (this.receiveData.provinceId=== '') {
                this.receiveData.provinceId=0
            }
        },
        computed:{
            cityList: function () {
                ...
                this.addList.filter(item => {
                    if (item.area_id === this.receiveData.provinceId) {
                        this.receiveData.province = item.area_name;
                        return cityList = item.city_list;
                    }
                    if (Number(this.receiveData.provinceId)=== 0) {
                        this.receiveData.cityId = 0;
                        this.receiveData.countyId = 0
                    }
                    if (this.receiveData.provinceId=== '') {
                        this.receiveData.provinceId=0
                    }
            ...
            countyList(){
                ...
                this.cityList.filter(item=> {
                    if (item.area_id === this.receiveData.cityId) {
                        this.receiveData.city = item.area_name;
                        return countyList=item.county_list;
                    }
                    if(Number(this.receiveData.cityId)===0){
                        this.receiveData.countyId=0
                    }
                });
                return countyList;
        ...
        methods:{
            closeFn(){
                this.$emit('closefn');
                for(var a in this.receiveData){
                    this.receiveData[a]='';
                }
            },
            judgePhone(){
                this.phoneError = this.receiveData.phone.length !== 11;
            },
            setError(){
                this.phoneError = false
            },
            tabDefault(){
                this.receiveData.default=!this.receiveData.default;
            },
            judgeAllHandler(){
                this.judgeAll = !!(this.receiveData.name && this.receiveData.phone && !this.phoneError && this.receiveData.province && this.receiveData.city && this.receiveData.county && this.receiveData.add);
            },
            saveAddress(){
                let obj=Object.assign({},this.receiveData);
                if(this.judgeAll){
                    this.$store.commit('saveAddress',obj);
                    this.closeFn();
                }
        ...
        watch:{
            'receiveData.countyId'(){
                this.countyList.forEach(item=>{
                    if(item.area_id===this.receiveData.countyId){
                        this.receiveData.county = item.area_name;
                    }
                })
            },
            receiveData:{
                handler(){
                    this.judgeAllHandler();
                },
                deep:true
            }
        }
